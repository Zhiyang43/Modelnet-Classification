...
for split in range(nDataSplits):
    ...
    train_data = data.ModelNet40(num_points, partition = 'train', perturb = 0.1 * split)
    ...
    test_data = data.ModelNet40(num_points, partition = 'test', perturb = 0.1 * split)
    for thisModel in modelsGNN.keys():
            ...
            if split == 0:
                InitCost[modelName] = thisCostBest
            costBestDiff[modelName][split] = abs( costBest[modelName][split] - InitCost[modelName] )
    #             costBest[modelName][split] = [thisCostBest]
    #             costLast[modelName][split] = [thisCostLast]
    ...
    #     test_data = data.ModelNet40(num_points, partition = 'test', perturb = epsilon)
...
if doFigs and doSaveVars:
    ...
    for key in costBestDiff.keys():
        plt.errorbar(xTest, costBestDiff[key], yerr = 0,
                     linewidth = lineWidth,
                     marker = markerShape, markersize = markerSize)
