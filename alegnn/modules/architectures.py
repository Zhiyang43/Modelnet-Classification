...
class SelectionGNN(nn.Module):
    ...
    Initialization:
        ...
        Input:
            ...
            /** Graph structure **/
            GSO (torch): graph structures are decided by each input data
    ...
    def __init__(self,
        ...
        if self.coarsening and self.E == 1:
            ...
            self.N = [nSelectedNodes[0]] + nSelectedNodes # Number of nodes
        ...
        # for clarity of code.
        self.gfl = [] # Graph Filtering Layers
        self.GF =[None] * self.L
        for l in range(self.L):
            #\\ Graph filtering stage:
            self.GF[l] = gml.GraphFilter(self.F[l], self.F[l+1], self.K[l],
                                              self.E, self.bias)
            self.gfl.append(self.GF[l])
            #\\ Nonlinearity
            self.gfl.append(self.sigma())
            #\\ Pooling
            if self.coarsening:
                self.gfl.append(self.rho(self.alpha[l]))
            else:
                self.gfl.append(self.rho(self.N[l], self.N[l+1], self.alpha[l]))
                
        # And now feed them into the sequential
        self.GFL = gml.mySequential(*self.gfl) # Graph Filtering Layers, with forward inputs and outputs both set as (x, S)
    ...
    def forward(self, x, S_la):
        ...
        # forward function.
        output, _ = self.splitForward(x, S_la)
